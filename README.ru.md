...
В Details следуйте поддержанной рекомендации и условиям сопоставимого rerun.
`Record rerun outcome` сохраняет локальный feedback; при повторном открытии
рекомендация показывает последний сохранённый результат. Query Doctor не
выполняет изменение и не запускает ваш SQL повторно.
